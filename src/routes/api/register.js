...
function isAutoGeneratedNoEmail(correo) {
  return normalizeEmail(correo).includes('no-email');
}
...
function isPendingRegistrationUser(row, documento) {
  ...
  return (
    isAutoGeneratedNoEmail(correo) ||
    isPlaceholderEmail(correo) ||
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let canContinueRegistration = rows.length === 0 || shouldPromotePlaceholderAccount;
