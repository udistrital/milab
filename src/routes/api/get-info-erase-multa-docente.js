...
router.post('/', requireLaboratoristaTeacherEraseAccess, async function (req, res) {
  ...
  try {
    ...
    const query =
      'SELECT COUNT(*) AS multado FROM multa WHERE usuario_id_sancionado = $1 AND con_estado_multa = $2';
    ...
    if (con_multado) {
      const queryMultaInfo =
        'SELECT m.*, us.documento AS documento_sancionado, u.nombre AS ual, l.nombre AS nombre_laboratorista, l.documento AS cc_laboratorista FROM multa m LEFT JOIN usuario us ON us.id = m.usuario_id_sancionado LEFT JOIN ual u ON u.id_ual = m.id_ual LEFT JOIN laboratorista l ON l.documento = m.documento_laboratorista WHERE m.usuario_id_sancionado = $1 AND m.con_estado_multa = $2';
