...
async function fetchUpcomingPrestamos() {
...
async function fetchUpcomingPracticas() {
  const result = await pool.query(
    `
      SELECT
        rp.id,
...
async function markReminderAsSent(tableName, id) {
  await pool.query(
    `
      UPDATE ${tableName}
...
async function processPrestamoReminders(prestamos) {
  for (const prestamo of prestamos) {
    try {
      ...
      await markReminderAsSent('solicitud_prestamo', prestamo.id);
    } catch (error) {
...
async function processPracticaReminders(practicas) {
  for (const practica of practicas) {
    try {
      ...
      await markReminderAsSent('reserva_practica', practica.id);
    } catch (error) {
...
async function runReminderCycle() {
  const prestamos = await fetchUpcomingPrestamos();
  await processPrestamoReminders(prestamos);

  const practicas = await fetchUpcomingPracticas();
  await processPracticaReminders(practicas);
}
